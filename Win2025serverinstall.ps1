...
# 6. Importer des utilisateurs depuis un CSV
$CSVPath = "C:\Chemin\Vers\Utilisateurs.csv"
$Users = Import-Csv -Path $CSVPath

foreach ($User in $Users) {
    New-ADUser -Name $User.Name -GivenName $User.FirstName -Surname $User.LastName -SamAccountName $User.SamAccountName -UserPrincipalName $User.UserPrincipalName -Path $User.OU -AccountPassword (ConvertTo-SecureString $User.Password -AsPlainText -Force) -Enabled $true
}